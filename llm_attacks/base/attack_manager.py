...
class AttackPrompt(object):
    ...
    def logits(self, model, test_controls=None, return_ids=False):
        ...
        elif not isinstance(test_controls, list) or not test_controls:
...
class MultiPromptAttack(object):
    ...
    def test(self, workers, prompts, include_loss=False):
        ...
        return model_tests_jb, model_tests_mb, model_tests_loss, model_tests_str
    ...
    def log(self, step_num, n_steps, control, loss, runtime, model_tests, verbose=True):
        ...
        prompt_tests_jb, prompt_tests_mb, model_tests_loss, model_tests_str = list(map(np.array, model_tests))
        all_goal_strs = self.goals + self.test_goals
        all_workers = self.workers + self.test_workers
        tests = {
            all_goal_strs[i]:
            [
                (all_workers[j].model.name_or_path, prompt_tests_jb[j][i], prompt_tests_mb[j][i], model_tests_loss[j][i], model_tests_str[j][i])
...
def get_workers(params, eval=False):
    ...
    workers = [
        ModelWorker(
            ...
            tokenizers[i],
            conv_templates[i],
            params.devices[i] if isinstance(params.devices,list) else params.devices
        )
